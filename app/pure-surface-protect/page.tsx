...
export default function PureSurfaceProtectPage() {
  ...
  return <>
    ...
    <section className="section section--surface" id="technology"><div className="container"><SectionHeading eyebrow={t.techEyebrow} title={t.techTitle} /><div className="surface-protect-tech-grid"><SurfaceDiagram locale={locale} /><div className="surface-protect-steps">{t.steps.map(([title, copy], index) => <article key={title}><span>0{index + 1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></div></div></section>

    <section className="section" id="applications"><div className="container"><SectionHeading eyebrow={t.applicationsEyebrow} title={t.applicationsTitle} /><div className="surface-protect-applications">{t.applications.map(([title, copy]) => <article key={title}><div className="surface-protect-application-visual"><span>{title.split(' & ')[0]}</span></div><h3>{title}</h3><p>{copy}</p></article>)}</div></div></section>
