...
export default function PureSurfaceProtectPage() {
  ...
  return <>
    ...
    <section className="hero surface-protect-hero">
    ...
    <section className="section" id="challenge"><div className="container"><SectionHeading eyebrow={t.challengeEyebrow} title={t.challengeTitle} intro={t.challengeCopy} /><div className="surface-protect-four-grid">{t.challengeCards.map(([title]) => <article className="surface-protect-card" key={title}><h3>{title}</h3></article>)}</div><p className="surface-protect-statement">{t.challengeStatement}</p></div></section>

    <section className="section section--surface" id="technology"><div className="container"><SectionHeading eyebrow={t.techEyebrow} title={t.techTitle} /><div className="surface-protect-tech-grid"><div><AssetFigure file={visualFor(productVisuals.surfaceProtect.technology, locale)} alt={asset('Illustratives Technologieprinzip für reine Oberflächen: Substrat, dünne Hybridmatrix und Oberflächenbarriere', 'Illustrative technology principle for clean surfaces: substrate, thin hybrid matrix and surface barrier')} caption={asset('Technologieprinzip / illustrative Kommunikationsgrafik – keine maßstäbliche technische Zeichnung.', 'Technology principle / illustrative communication visual — not a scaled technical drawing.')} /></div><div className="surface-protect-steps">{t.steps.map(([title, copy], index) => <article key={title}><span>0{index + 1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></div></div></section>

    <section className="section" id="applications"><div className="container"><SectionHeading eyebrow={t.applicationsEyebrow} title={t.applicationsTitle} /><div className="surface-protect-applications">{t.applications.map(([title, copy], index) => <article key={title}>{index === 1 ? <AssetFigure file={visualFor(productVisuals.surfaceProtect.infrastructure, locale)} alt={asset('Illustratives Anwendungsvisual einer modernen Terminalarchitektur im Morgenlicht', 'Illustrative application visual of modern terminal architecture in morning light')} caption={asset('Infrastruktur / illustrative Kommunikationsgrafik – keine dokumentierte PURE-Referenz.', 'Infrastructure / illustrative communication visual — not a documented PURE reference.')} /> : index === 2 ? <AssetFigure file={visualFor(productVisuals.surfaceProtect.industry, locale)} alt={asset('Illustratives Anwendungsvisual einer hochwertigen Edelstahl- und Industrieoberfläche', 'Illustrative application visual of a high-quality stainless-steel industrial surface')} caption={asset('Industrie / illustrative Kommunikationsgrafik – keine dokumentierte PURE-Referenz.', 'Industry / illustrative communication visual — not a documented PURE reference.')} /> : <div className="surface-protect-application-visual"><span>{title.split(' & ')[0]}</span></div>}<h3>{title}</h3><p>{copy}</p></article>)}</div></div></section>
