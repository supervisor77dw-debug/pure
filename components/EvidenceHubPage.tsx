...
export function EvidenceHubPage({ locale }: { locale: Locale }) {
  ...
  return (
    <>
      ...
      <section className="section section--surface"><div className="container"><div className="evidence-dossier-grid"><article id="water-vapor-diffusion" className="evidence-dossier evidence-anchor evidence-anchor--water"><EvidenceBadge evidenceClass="A" locale={locale} /><h3>{de ? 'Wasserdampf / Bauphysik' : 'Water vapour / building physics'}</h3><div className="evidence-dossier__metrics"><strong>V = 25.6 <small>g/(m²·d)</small></strong><strong>sd = 0.76 <small>m</small></strong><strong>V2 <small>/ medium</small></strong><strong>μ ≈ 95–109 <small>{de ? 'abgeleitet' : 'derived'}</small></strong></div><p>{de ? 'Geprüft an PureThermo-Musterplatten bei ca. 7–8 mm. Der μ-Wert ist aus sd und Schichtdicke abgeleitet und nicht direkt gemessen.' : 'Tested on PureThermo sample plates at approximately 7–8 mm. The μ value is derived from sd and layer thickness and was not measured directly.'}</p><small>{de ? 'Finale Rezeptur- und Systemzuordnung für die jeweilige Produktversion bestätigen.' : 'Final formulation and system mapping must be confirmed for the respective product version.'}</small></article><article id="fire-behavior" className="evidence-dossier evidence-anchor evidence-anchor--fire"><EvidenceBadge evidenceClass="A" locale={locale} /><h3>{de ? 'Feuer: Einzelprüfung und SBI-Pfad' : 'Fire: individual evidence and SBI pathway'}</h3><div className="evidence-dossier__split"><div><strong>{de ? 'Kleinflamme' : 'Small flame'}</strong><p>{de ? 'Flammenhöhe ca. 50–60 mm · 150-mm-Marke nicht erreicht · kein Nachbrennen · keine brennenden Tropfen im geprüften Aufbau.' : 'Flame height approx. 50–60 mm · 150 mm mark not reached · no afterflame · no burning droplets in the tested setup.'}</p></div><div><strong>{de ? 'Indikativer SBI' : 'Indicative SBI'}</strong><p>FIGRA 885 / 950 W/s · THR600s 8.13 / 9.70 MJ</p><small>{de ? 'Keine allgemeine Brandklasse ableiten.' : 'Do not derive a general fire classification.'}</small></div></div></article></div></div></section>
