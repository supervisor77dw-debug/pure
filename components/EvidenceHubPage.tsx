  ...
  return (
    <>
      ...
      <section className="section section--surface"><div className="container"><div className="section-heading section-heading--narrow"><span className="section-heading__eyebrow">{de ? 'Werte & Prüfpfade' : 'Values & test paths'}</span><h2>{de ? 'Nicht nur Lambda.' : 'More than lambda.'}</h2><p>{de ? 'Der Hub bereitet die wichtigsten Evidenzbereiche für Produkt- und Systembewertungen vor.' : 'The hub prepares the key evidence areas for product and system assessment.'}</p></div><div className="evidence-card-list">
        ...
        <EvidenceHubCard locale={locale} badge="A" title={{ de: 'Wasserdampfdiffusion', en: 'Water vapour diffusion' }} source="DIN EN ISO 7783-2 · DIN EN 1062-1" statement={de ? 'V = 25,6 g/(m²·d), sd = 0,76 m, V2 - mittel bei ca. 7-8 mm Prüfschicht.' : 'V = 25.6 g/(m²·d), sd = 0.76 m, V2 - medium at approx. 7-8 mm test layer.'} limit={de ? 'μ ca. 95-109 ist abgeleitet und darf nicht als direkte μ-Messung dargestellt werden.' : 'μ approx. 95-109 is derived and must not be presented as a direct μ measurement.'} href={evidenceDetailHref} />
        ...
        <EvidenceHubCard locale={locale} badge="D" title={{ de: 'PURE THERMO Brand-Einzelprüfung', en: 'PURE THERMO individual fire test' }} source={de ? 'Separat von Kombinationsroute zu bewerten' : 'To be assessed separately from combination route'} statement={de ? 'Brandverhalten darf nur für den konkret geprüften Aufbau beschrieben werden.' : 'Fire behaviour may only be described for the specifically tested build-up.'} limit={de ? 'Keine Ableitung einer PURE THERMO + PURE FIRE Kombinationsklasse.' : 'No derivation of a PURE THERMO + PURE FIRE combination class.'} href={fireRouteHref} />
        <EvidenceHubCard locale={locale} badge="D" title={{ de: 'PURE FIRE SBI-Einzelprüfung', en: 'PURE FIRE individual SBI test' }} source={de ? 'Einzelprüfung / Prüfpfad getrennt führen' : 'Individual test / test path kept separate'} statement={de ? 'SBI-Ergebnisse gehören zur geprüften Einzelkonfiguration.' : 'SBI results belong to the tested individual configuration.'} limit={de ? 'Nicht mit PURE THERMO zu einer Systemklasse zusammenziehen.' : 'Do not merge with PURE THERMO into a system class.'} href={fireRouteHref} />
...
function EvidenceHubCard({ locale, badge, title, source, statement, limit, href, chart = false }: { locale: Locale; badge: 'A' | 'B' | 'C' | 'D'; title: { de: string; en: string }; source: string; statement: string; limit: string; href: string; chart?: boolean }) {
  ...
  return (
    <article className="evidence-hub-card">
      <EvidenceBadge evidenceClass={badge} />
      <div>
        <h3>{title[locale]}</h3>
