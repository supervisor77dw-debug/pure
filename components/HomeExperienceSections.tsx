...
const products = [
  { slug: 'pure-thermo', name: 'PURE THERMO', category: { de: 'Thermische Funktionsschicht', en: 'Thermal functional layer' }, copy: { de: 'Dünne Beschichtung für Bauteile, Details und begrenzte Aufbauhöhen.', en: 'Thin coating for components, details and limited build-up heights.' }, asset: { de: 'Moderne Gebäudeisolierung im Querschnitt.png', en: 'Moderne Gebäudeisolierung im Querschnitt_eng.png' }, href: { de: 'produkte/pure-thermo', en: 'products/pure-thermo' } },
  { slug: 'pure-liquid-heat', name: 'PURE LIQUID HEAT', category: { de: 'Elektrische Wärme', en: 'Electrical heat' }, copy: { de: 'Flächige elektrische Wärme als Material- und Modulroute.', en: 'Surface electrical heat as material and module route.' }, asset: { de: 'LH_CONCEPT_PureHeat_Prism_01.jpg', en: 'LH_CONCEPT_PureHeat_Prism_01.jpg' }, href: { de: 'pure-liquid-heat', en: 'products/pure-liquid-heat' } },
  { slug: 'pure-floor-protect', name: 'PURE FLOOR PROTECT', category: { de: 'Bodenoberflächen', en: 'Floor surfaces' }, copy: { de: 'Oberflächenschutz für stark genutzte Bodenbereiche.', en: 'Surface protection for heavily used floor areas.' }, asset: { de: 'Sauberer Boden für starke Räume.png', en: 'Sauberer Boden für starke Räume.png' }, href: { de: 'produkte/pure-floor-protect', en: 'products/pure-floor-protect' } },
  { slug: 'pure-fire-protect', name: 'PURE FIRE PROTECT', category: { de: 'Brandschutz', en: 'Fire protection' }, copy: { de: 'Brandschutzroute mit klarer Prüf- und Klassifizierungslogik.', en: 'Fire-protection route with clear testing and classification logic.' }, asset: { de: '01_fire_hero_wood_architecture.jpeg', en: '01_fire_hero_wood_architecture.jpeg' }, href: { de: 'produkte/pure-fire-protect', en: 'products/pure-fire-protect' } },
  { slug: 'pure-surface-protect', name: 'PURE SURFACE PROTECT', category: { de: 'Oberflächenschutz', en: 'Surface protection' }, copy: { de: 'Schutzprinzip für Architektur-, Infrastruktur- und Industrieoberflächen.', en: 'Protection principle for architecture, infrastructure and industrial surfaces.' }, asset: { de: 'PURE Surface Protect am Flughafen Terminal.png', en: 'PURE Surface Protect am Flughafen Terminal.png' }, href: { de: 'produkte/pure-surface-protect', en: 'products/pure-surface-protect' } },
  { slug: 'pure-water-protect', name: 'PURE WATER PROTECT', category: { de: 'Unterwasser / Marine', en: 'Underwater / marine' }, copy: { de: 'Easy-to-clean-Route für maritime und dauerhaft wasserberührte Oberflächen.', en: 'Easy-to-clean route for marine and water-exposed surfaces.' }, asset: { de: '01_water_hero_marine.jpeg', en: '01_water_hero_marine.jpeg' }, href: { de: 'produkte/pure-water-protect', en: 'products/pure-water-protect' } },
  { slug: 'pure-wood-protect', name: 'PURE WOOD PROTECT', category: { de: 'Holzoberflächen', en: 'Wood surfaces' }, copy: { de: 'Schutzroute für Holzbauteile und sichtbare Holzoberflächen.', en: 'Protection route for timber components and visible wood surfaces.' }, asset: { de: '01_wood_asset_page01.jpeg', en: '01_wood_asset_page01.jpeg' }, href: { de: 'produkte/pure-wood-protect', en: 'products/pure-wood-protect' } },
  { slug: 'pure-boat-protect', name: 'PURE BOAT PROTECT', category: { de: 'Yacht & Boot', en: 'Yacht & boat' }, copy: { de: 'Oberflächenroute für Yacht- und Bootsbereiche.', en: 'Surface route for yacht and boat areas.' }, asset: { de: '01_boat_asset_page01.jpeg', en: '01_boat_asset_page01.jpeg' }, href: { de: 'produkte/pure-boat-protect', en: 'products/pure-boat-protect' } }
] as const;
...
const evidencePreview = [
  ...
  { title: { de: 'PURE THERMO Brand-Einzelprüfung', en: 'PURE THERMO individual fire test' }, evidence: 'D', source: { de: 'Getrennt von Kombinationsroute zu betrachten', en: 'To be considered separately from combination route' }, statement: { de: 'Einzelprüfung darf nur für den geprüften Aufbau bewertet werden.', en: 'Individual test may only be assessed for the tested build-up.' }, limit: { de: 'Keine Kombinationsklasse ableiten.', en: 'Do not derive a combination class.' } },
  { title: { de: 'PURE FIRE SBI-Einzelprüfung', en: 'PURE FIRE individual SBI test' }, evidence: 'D', source: { de: 'Einzelroute / Prüfpfad', en: 'Individual route / test path' }, statement: { de: 'SBI-Ergebnisse gehören zur jeweiligen geprüften Konfiguration.', en: 'SBI results belong to the tested configuration.' }, limit: { de: 'Nicht mit PURE THERMO zu einer Klasse zusammenziehen.', en: 'Do not merge with PURE THERMO into one class.' } },
    ...
    <section className="section section--surface" id={de ? 'produkte' : 'portfolio'}>
      <div className="container">
        ...
        <div className="portfolio-matrix">
          {products.map((product) => <Link className="portfolio-card" href={localePath(locale, product.href[locale])} key={product.slug}><AssetFigure file={product.asset[locale]} alt={product.name} variant="plain" /><span>{product.category[locale]}</span><h3>{product.name}</h3><p>{product.copy[locale]}</p></Link>)}
        </div>
      </div>
    </section>
  );
...
export function PartnershipPanel({ locale }: { locale: Locale }) {
  ...
  return <section className="section" id={de ? 'partnerschaft' : 'partnership'}><div className="container"><div className="section-heading section-heading--narrow"><span className="section-heading__eyebrow">{de ? 'Partnerschaft' : 'Partnership'}</span><h2>{de ? 'Die Zusammenarbeit beginnt mit der richtigen Frage.' : 'Collaboration starts with the right question.'}</h2></div><div className="partner-grid partner-grid--actions">{items.map(([title, audience, copy, action]) => <article className="partner-card" key={title}><span className="partner-card__icon" aria-hidden="true">{audience.slice(0, 2).toUpperCase()}</span><small>{audience}</small><h3>{title}</h3><p>{copy}</p><Link href={localePath(locale, de ? 'produkte/pure-thermo' : 'products/pure-thermo')}>{action}</Link></article>)}</div><div className="hero__ctas" style={{ justifyContent: 'center', marginTop: 'var(--space-4)' }}><Link className="btn btn--primary" href={localePath(locale, de ? 'produkte/pure-thermo' : 'products/pure-thermo')}>{de ? 'Projekt besprechen' : 'Discuss project'}</Link></div></div></section>;
}

export function FinalDecisionCta({ locale }: { locale: Locale }) {
  const de = locale === 'de';
  return <section className="section section--surface"><div className="container"><div className="final-cta"><p className="final-cta__lead">{de ? 'Verstehen. Berechnen. Prüfen. Besprechen.' : 'Understand. Calculate. Verify. Discuss.'}</p><h2>{de ? 'Sprechen Sie mit uns über Ihre Anwendung.' : 'Talk to us about your application.'}</h2><div className="decision-links"><Link href={localePath(locale, de ? 'produkte/pure-thermo#u-wert-rechner' : 'products/pure-thermo#u-wert-rechner')}>{de ? 'Bauteil berechnen' : 'Calculate component'}</Link><Link href={localePath(locale, de ? 'systeme' : 'systems')}>{de ? 'Systeme ansehen' : 'View systems'}</Link><Link href={localePath(locale, de ? 'nachweise' : 'evidence')}>{de ? 'Nachweise prüfen' : 'Check evidence'}</Link></div><div className="hero__ctas" style={{ justifyContent: 'center' }}><Link className="btn btn--primary" href={localePath(locale, de ? 'produkte/pure-thermo' : 'products/pure-thermo')}>{de ? 'Projekt besprechen' : 'Discuss project'}</Link></div></div></div></section>;
}
